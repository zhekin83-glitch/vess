"""Self-sufficient runtime dependency bootstrap for manga-studio.
...
1. **Probe** the three places host-managed deps could already live and
   ...
   - ``~/.vess/modules/manga-studio/site-packages/`` — host's
...
2. **Import-or-install**: if the import still fails, run ``pip install``
   ...
   ``~/.vess/modules/manga-studio/site-packages/`` so the install
...
def _patch_simplejson_jsondecodeerror() -> bool:
    ...
    if errors_mod is not None and not hasattr(errors_mod, "JSONDecodeError"):
        try:
            ...
            pass
    logger.info("manga-studio: patched simplejson.JSONDecodeError for requests")
    return True
...
def _candidate_site_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    cands.append(_openakita_root() / "modules" / "manga-studio" / "site-packages")
...
def _pip_install(specs: list[str], target: Path) -> tuple[bool, str]:
    ...
    for url, trusted in _PIP_MIRRORS:
        ...
        cmd.extend(specs)
        logger.info("manga-studio dep install via %s: %s", url, specs)
        try:
            proc = subprocess.run(
                cmd,
                env=env,
        ...
        if proc.returncode == 0:
            logger.info("manga-studio dep install ok via %s", url)
            return True, ""
    ...
    *,
    ...
    if appended:
        ...
        try:
            mod = importlib.import_module(import_name)
            _RESOLVED.add(import_name)
            logger.info("manga-studio: %s imported after sys.path += %s", label, appended)
            return mod
        except ImportError:
            pass
    ...
    with _INSTALL_LOCK:
        ...
        _RESOLVED.add(import_name)
        logger.info("manga-studio: %s installed and imported (%s)", label, target)
        return mod
    ...
    *,
    ...
    def _worker() -> None:
        ...
        try:
            ...
            _log_dep(dep_id, msg)
            logger.warning("manga-studio dep %s install failed: %s", dep_id, exc)
        finally:
            state["busy"] = False
    ...
    t = threading.Thread(
        target=_worker,
        name=f"manga-studio-dep-{dep_id}",
        daemon=True,
    )
    t.start()
    return probe_dependency(dep_id, import_name, plugin_dir=plugin_dir)
...
def preinstall_async(
    ...
    *,
    ...
    def _worker() -> None:
        for import_name, pip_spec in specs:
            try:
                ...
                logger.info(
                    "manga-studio preinstall %s skipped: %s",
                    import_name,
                    e,
                )
    ...
    t = threading.Thread(
        target=_worker,
        name="manga-studio-dep-bootstrap",
        daemon=True,
    )
    t.start()
