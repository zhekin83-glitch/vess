...
# ── footage-gate extension: FFmpeg capability probe ───────────────────────
#
# Beyond "ffmpeg is installed?" footage-gate's auto_color / cut_qc / silence_cut
# pipelines depend on a *specific* set of FFmpeg filters and a minimum version
# (4.4, where ``signalstats`` started reporting bit-depth-aware Y averages and
# the ``zscale + tonemap`` HDR→SDR chain became universally available). Older
# releases either lack ``signalstats`` (3.x), reject ``eq=gamma=`` for HDR
# input without an explicit tonemap pre-stage (4.0–4.2 — see video-use PR #6),
# or compile without ``--enable-libzimg`` so ``tonemap_zscale`` is missing
# entirely (some Windows static builds shipped via choco prior to 2022).
#
# We expose a single ``probe_ffmpeg_capabilities`` helper used by
# ``FootageGatePlugin._setup_environment`` at every task launch. The returned
# dict mirrors ``SystemDepsManager.detect()`` shape so the same Settings UI
# block can render both the install-state badge and the capability matrix.
#
# Filters probed (all required for the four canonical modes):
#   * ``signalstats`` — auto_color frame stats sampling (Y/U/V averages,
#     bit-depth, min/max). Source: video-use helpers/grade.py L78–143.
#   * ``eq``          — auto_color colour grading filter chain (contrast /
#     gamma / saturation). Source: video-use helpers/grade.py L235–291.
#   * ``subtitles``   — cut_qc subtitle_overlay_check + future burn-in.
#     Source: video-use helpers/render.py L471–476.
#   * ``tonemap`` / ``zscale`` — HDR→SDR fallback for HDR sources before
#     ``eq`` is applied. Source: video-use PR #6 (defensive fix for HDR
#     tonemap regression). Either ``tonemap`` (libavfilter built-in) OR the
#     ``zscale + tonemap_zscale`` pair satisfies the requirement.
#
# We deliberately do NOT exec ``ffmpeg -filters`` and grep the human-readable
# table — its column layout has shifted between 4.x and 6.x and is locale-
# sensitive. Instead we use ``ffmpeg -hide_banner -h filter=<name>`` which
# returns exit code 0 + a "Filter <name>" line iff the filter is available,
# and is stable from 3.4+ to 7.x.

REQUIRED_FFMPEG_VERSION: tuple[int, int] = (4, 4)

REQUIRED_FFMPEG_FILTERS: tuple[str, ...] = (
    "signalstats",
    "eq",
    "subtitles",
)

# Either ``tonemap`` OR the ``zscale``+``tonemap_zscale`` pair satisfies the
# HDR fallback; we fold them into one logical capability called ``hdr_tonemap``
# so the UI shows a single row.
HDR_TONEMAP_ALTERNATIVES: tuple[tuple[str, ...], ...] = (
    ("tonemap",),
    ("zscale", "tonemap_zscale"),
)


def _parse_ffmpeg_version(text: str) -> tuple[int, int] | None:
    """Extract (major, minor) from an ``ffmpeg -version`` blob.

    Handles the common forms — official releases (``ffmpeg version 4.4.2``),
    distro builds with suffix (``ffmpeg version 4.4-1ubuntu0.1+esm1``), and
    git/N-builds where the leading token is ``Nxxxxx`` (``ffmpeg version
    N-100123-g4f5a6b7``). Git builds return ``None`` so the caller can warn
    "version unknown" rather than guess a satisfied minimum it cannot prove.
    """
    if not text:
        return None
    ...
    if not m:
        return None
    raw = m.group(1)
    # Git / nightly builds use ``N-<rev>-g<sha>`` or ``n<x.y.z>``; we cannot
    # confidently map those to (major, minor) so return None and let the
    # caller treat "unknown" as permissive (with a Settings UI warning).
    if raw.startswith("N-") or (raw.startswith("n") and raw[1:2].isdigit()):
        return None
...
def _has_ffmpeg_filter(ffmpeg_path: str, filter_name: str) -> bool:
    """Return True iff ``filter_name`` is compiled into ``ffmpeg_path``.

    Uses ``ffmpeg -hide_banner -h filter=<name>``: the binary prints
    ``"Filter <name>"`` on the first line when present, and ``"Unknown
    filter '<name>'."`` otherwise (exit code is 0 in both cases on 4.x+,
    1 on some 3.x builds — we rely on the stdout text, not the exit code).
    """
    if not ffmpeg_path or not filter_name:
        return False
    try:
        proc = subprocess.run(
        ...
        logger.debug("filter probe %s failed: %s", filter_name, exc)
        return False
    blob = (proc.stdout or "") + "\n" + (proc.stderr or "")
    return f"Filter {filter_name}" in blob


def probe_ffmpeg_capabilities(ffmpeg_path: str) -> dict[str, Any]:
    """Probe FFmpeg version and required filters.

    Returns a dict shaped to slot directly into the Settings page payload
    next to ``SystemDepsManager.detect("ffmpeg")``::

        {
            "ok": bool,           # all required capabilities satisfied?
            "version": (4, 4) | None,
            "version_str": "4.4.2" or "" if unknown,
            ...
            "hdr_tonemap": bool,  # at least one alternative present
            "hdr_tonemap_via": "tonemap" | "zscale+tonemap_zscale" | "",
            "missing": ["..."],   # human-readable list of unmet requirements
        }

    Never raises — every failure mode (binary missing, hangs, parse errors)
    is folded into a structured dict so the caller can present a clear
    Settings-page banner without try/except plumbing.
    """
    out: dict[str, Any] = {
        "ok": False,
        "version": None,
        ...
        "filters": dict.fromkeys(REQUIRED_FFMPEG_FILTERS, False),
        "hdr_tonemap": False,
        "hdr_tonemap_via": "",
        "missing": [],
    }
    if not ffmpeg_path:
        out["missing"].append("ffmpeg_not_found")
        return out
    ...
    try:
        ...
        out["missing"].append("ffmpeg_version_unreadable")
        return out

    blob = (proc.stdout or "") + "\n" + (proc.stderr or "")
    parsed = _parse_ffmpeg_version(blob)
    out["version"] = parsed
    if parsed is not None:
        out["version_str"] = f"{parsed[0]}.{parsed[1]}"
        out["version_satisfied"] = parsed >= REQUIRED_FFMPEG_VERSION
        if not out["version_satisfied"]:
            out["missing"].append(
                f"ffmpeg_version_too_old:{parsed[0]}.{parsed[1]}<"
                f"{REQUIRED_FFMPEG_VERSION[0]}.{REQUIRED_FFMPEG_VERSION[1]}"
            )
    else:
        # Unknown (git/N-build) — be permissive, just flag in the UI.
        out["version_satisfied"] = True

    # Filter probes.
    for name in REQUIRED_FFMPEG_FILTERS:
        present = _has_ffmpeg_filter(ffmpeg_path, name)
        out["filters"][name] = present
        if not present:
            out["missing"].append(f"filter_missing:{name}")

    # HDR tonemap fallback (at least one alternative must be fully present).
    for alt in HDR_TONEMAP_ALTERNATIVES:
        if all(_has_ffmpeg_filter(ffmpeg_path, n) for n in alt):
            out["hdr_tonemap"] = True
            out["hdr_tonemap_via"] = "+".join(alt)
            break
