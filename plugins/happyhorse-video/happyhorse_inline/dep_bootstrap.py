"""Self-sufficient runtime dependency bootstrap for happyhorse-video.
...
1. **Probe** the three places host-managed deps could already live and
   ...
   - ``~/.vess/modules/happyhorse-video/site-packages/`` — host's
...
2. **Import-or-install**: if the import still fails, run ``pip install``
   ...
   ``~/.vess/modules/happyhorse-video/site-packages/`` so the install
...
def _patch_simplejson_jsondecodeerror() -> bool:
    ...
    if errors_mod is not None and not hasattr(errors_mod, "JSONDecodeError"):
        try:
            ...
            pass
    logger.info("happyhorse-video: patched simplejson.JSONDecodeError for requests")
    return True
...
def _candidate_site_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    cands.append(_openakita_root() / "modules" / "happyhorse-video" / "site-packages")
    ...
    *,
    ...
    if appended:
        ...
        try:
            mod = importlib.import_module(import_name)
            _RESOLVED.add(import_name)
            logger.info("happyhorse-video: %s imported after sys.path += %s", label, appended)
            return mod
        except ImportError:
            pass
    ...
    with _INSTALL_LOCK:
        ...
        _RESOLVED.add(import_name)
        logger.info("happyhorse-video: %s installed and imported (%s)", label, target)
        return mod
    ...
    *,
    ...
    def _worker() -> None:
        ...
        try:
            ...
            _log_dep(dep_id, msg)
            logger.warning("happyhorse-video dep %s install failed: %s", dep_id, exc)
        finally:
            state["busy"] = False
    ...
    t = threading.Thread(
        target=_worker,
        name=f"happyhorse-video-dep-{dep_id}",
        daemon=True,
    )
    t.start()
    return probe_dependency(dep_id, import_name, plugin_dir=plugin_dir)
...
def preinstall_async(
    ...
    *,
    ...
    t = threading.Thread(
        target=_worker,
        name="happyhorse-video-dep-bootstrap",
        daemon=True,
    )
    t.start()
