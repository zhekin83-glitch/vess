...
clip-sense in 0.7.0; see ``clip_sense_inline/__init__.py``. Replaces the
synchronous ``Path.rglob()`` pattern used in the original seedance code path
...
def _walk_sync(
    ...
    *,
    ...
    for root in roots:
        ...
        for p in iterator:
            try:
                if skip_hidden and any(part.startswith(".") for part in p.parts[root_depth:]):
                    continue
                if not p.is_file():
                    continue
                size = p.stat().st_size
