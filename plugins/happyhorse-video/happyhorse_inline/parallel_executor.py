...
into happyhorse-video in 0.7.0; see ``happyhorse_inline/__init__.py``. Used by
``long_video.ChainGenerator.generate_chain`` (parallel mode) to fan out
per-segment Ark calls with bounded concurrency and a guaranteed result row
per input (no silent skip).
"""
