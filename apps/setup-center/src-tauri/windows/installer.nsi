...
LangString envDetectedLabel ${LANG_SIMPCHINESE} "检测到已有的器灵 Vess 数据，旧版环境组件将在安装过程中自动清理。"
LangString envDetectedLabel ${LANG_ENGLISH} "Existing Vess data detected. Legacy environment components will be cleaned up automatically during installation."
LangString envCleanCheckbox ${LANG_SIMPCHINESE} "清除所有用户数据（含旧版 .openakita / 新版 .vess：聊天记录、工作区、个人设置等）"
LangString envCleanCheckbox ${LANG_ENGLISH} "Remove all user data (legacy .openakita / .vess: chat history, workspaces, personal settings, etc.)"
...
LangString envCleanHint ${LANG_SIMPCHINESE} "提示：默认勾选以清理旧数据。若只是升级并保留配置，请取消勾选。清除需二次确认。"
LangString envCleanHint ${LANG_ENGLISH} "Tip: Checked by default to clean old data. Uncheck to keep settings when upgrading. Removal requires confirmation."
...
; The full locked-file list is written to %USERPROFILE%\.vess\logs\
; install_locked_<timestamp>.log for post-mortem inspection.
LangString installAbortLocked ${LANG_SIMPCHINESE} "提示：检测到部分 OpenAkita 文件可能仍被占用（杀毒软件扫描或 Windows 索引常见），安装将继续。如最终失败，请查看 %USERPROFILE%\.vess\logs\install_locked_*.log，关闭相关 openakita-* 进程后重试。"
LangString installAbortLocked ${LANG_ENGLISH} "Notice: Some OpenAkita files may still be in use (commonly AV scans or Windows indexing). Installation will proceed. If it ultimately fails, see %USERPROFILE%\.vess\logs\install_locked_*.log, close related openakita-* processes, and retry."
